...
const seed = ({ categoryData, commentData, reviewData, userData }) => {
    return db
        ...
        .then(() => {
        const insertCategoriesQueryStr = format("INSERT INTO categories (slug, description) VALUES %L;", categoryData.map(({ slug, description }) => [slug, description]));
        ...
        const insertReviewsQueryStr = format("INSERT INTO reviews (title, category, designer, owner, review_body, review_img_url, created_at, votes) VALUES %L RETURNING *;", formattedReviewData.map(({ title, category, designer, owner, review_body, review_img_url, created_at, votes, }) => [
            title,
            category,
            designer,
            owner,
        ...
        const insertCommentsQueryStr = format("INSERT INTO comments (body, author, review_id, votes, created_at) VALUES %L;", formattedCommentData.map(({ body, author, review_id, votes = 0, created_at }) => [
            body,
            author,
            review_id,
            votes,
